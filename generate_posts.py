...
def call_claude_api(prompt, api_key):
    ...
    response = client.messages.create(
        ...
        max_tokens=4096,
        system="あなたはX（旧Twitter）のコンテンツ戦略の専門家です。指示通りのJSON形式のみを返してください。JSONの前後に説明文を入れないでください。",
...
def main():
    parser = argparse.ArgumentParser(description='X投稿文をClaude APIで自動生成')
    ...
    raw = call_claude_api(prompt, api_key)
