...
| 会話クリック（詳細を見る） | +11.0 |
| 会話クリック＋2分以上滞在（Dwell） | +10.0 |
| リツイート（Repost） | +1.0 |
| いいね（Like） | +0.5 |
| ネガティブ反応（ブロック・ミュート等） | -74.0 |
| スパム報告（Report） | -369.0 |

⚠️ 【2026年5月アップデート】重要な変更点:
- **ReplyScorer導入**: リプライはGrok VLMが品質評価。「挨拶のみ」「スタンプのみ」など内容の薄いリプライは加点されない。意味のある会話が成立しているかが重要。
- **新指標が追加**: P(share)・P(follow_author)・P(dwell)・P(quote) も正式にスコアリング対象に。
- **slop_score（AI量産コンテンツ検出）**: BangerInitialScreenがAIで量産された手抜き投稿を検出しスコアを下げる。
- **SpamEapiLowFollowerClassifier**: フォロワー数が少ない段階でのインプレッション狙いのリプライ営業はスパム判定リスクあり。
- **トピック一貫性（Phoenix topics）**: 専門領域への継続的な投稿がOut-of-Networkでの発見確率を高める。

## 投稿最適化チェックリスト（2026年5月版）

1. **リプライの質**: 「会話が成立しているか？」を意識した問いかけを設計する（量より質）
2. **Dwell（1行目で止める）**: 最初の1文だけで「読む価値がある」と判断させる強い導入
3. **シェア・引用したくなる余白**: 他者が意見を添えて引用リポストしたくなる構成にする
4. **フォロー価値**: 「このアカウントをフォローすれば得する」と感じさせる専門性を示す
5. **プロフィール誘導**: プロフィールを見に行きたくなる一行を含める（+12.0）
6. **スレッドで続きを読ませる構成**: 会話クリック+滞在を誘発（+11.0/+10.0）
7. **質の高い画像**: タップして拡大したくなるビジュアルを添付
8. **URLは本文に入れない**: スパム判定ペナルティ回避（自己リプライへ移動）
9. **AI量産感を排除**: テンプレート的な文体・絵文字の乱用・同じ構成の繰り返しを避ける
10. **IMA検定の専門トピックに絞る**: Webマーケ・AI活用・デジタルマーケの一貫性を保つ
"""
...
**必須ルール（全投稿共通）**:
1. **最初の1文で立ち止まらせる**: 1行目だけで「読む価値がある」と判断できる強いフックを入れる（Dwell最大化）
2. **質の高い問いかけ**: 末尾に具体的な問いかけを入れ、意味のある会話が成立するよう設計する（「はい/いいえ」ではなく経験や意見を引き出す）
3. **シェア・引用したくなる余白**: 他者が自分の意見を添えて引用リポストしたくなる「問題提起」「事実の提示」を意識する
4. **フォロー価値の提示**: 「このアカウントをフォローすれば〇〇が学べる」と伝わる専門性・独自性を示す
5. **プロフィール誘導**: 「プロフィールのリンクから👇」などプロフィールを見に行きたくなる一行を含める
6. 本文にURLを絶対に入れない（申込みURLが必要な場合はcontentの最後の要素として自己リプライ形式で追加）
7. 1本目は必ずスレッド形式（3〜4ツイート）にする
8. ハッシュタグは1投稿あたり2〜3個のみ
9. 各投稿のテーマは重複させない
10. **AI量産感を出さない**: テンプレート的な文体・同じ絵文字の多用・毎回同じ構成を避ける。人間が書いたような自然な語り口にする
11. **IMA検定の専門領域に絞る**: Webマーケ・AI活用・デジタルマーケティングの専門性を一貫させ、Out-of-Networkでの発見率を高める
